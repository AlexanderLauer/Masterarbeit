...
The \emph{truncated condition at layer} $k$ of $c$, $\cut{k}{c}$, is defined as
...
$$\ic{k}{c}{C'} := \begin{cases}
							c & \text{if $\scond{k}{c}$ is existentially bound}\\
							\repl{k}{c}{\exists(a_k^r:C_{k} \inj C', \true)} &\text{otherwise}
					   \end{cases}$$
