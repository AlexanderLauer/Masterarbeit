...
If a graph $G$ does not satisfy the condition up to layer $k := \maxc{G} + 2$ of a given constraint $c$, with $k$ being even, there does exists a graph $C' \in \mathcal{U}(C_{k-1}, C_k)$, such that $G$ satisfies $\parcond(k,c,C')$, note that $G$ always satisfies $\parcond(k,c,C_{k-1})$. 
