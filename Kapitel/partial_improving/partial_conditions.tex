...
Let a condition $c$ in ANF be given. The \emph{subcondition at layer $0 \leq k \leq \nlvl(c)$}, denoted by $\scond{k}{c}$, is the subcondition $d$ of $c$ with $\lay(d) = k$. 
\end{definition}
...
Let us first introduce an operator which allows to replace a subcondition $\scond{k}{c}$ by an arbitrary condition over $C_k$, called \emph{replacement at layer}.
...
The \emph{replacement of layer $k$ in $c$ by $e$}, denoted by $\repl{k}{c}{e}$, is defined recursively as
\begin{equation*}
	\repl{k}{c}{e} := 
		\begin{cases}
			e & \text{if $k = 0$} \\
...
\begin{example}
Consider the conditions $c := \forall(a_0: C_0 \inj C_1, \exists(a_1: C_1 \inj C_2, \true))$ and $d = \exists(a'_1: C_1 \inj C_3, e)$. 
The replacement of layer $1$ in $c$ by $d$ is given by
$$\repl{1}{c}{d} = \forall(a_0: C_0 \inj C_1, \exists(a'_1: C_1 \inj C_3, e)).$$ 
\end{example}

We now define \emph{truncated conditions} using the concept of replacement at layer.
Intuitively, a condition is truncated at a particular layer by replacing the subcondition at that layer with $\true$ or $\false$, depending on which quantifier the replaced subcondition is bound by.


\begin{definition}[\textbf{truncated condition}]
	Let a condition $c$ in UANF be given. 
	The \emph{truncated condition of $c$ at layer $-1 \leq k < \nlvl(c)$ }, 
...
The largest $-1 \leq k < \nlvl(c)$ such that $G \models_k c$ and there is no $k < j < \nlvl(c)$ with $G \models_j c$, called the \emph{biggest satisfied layer}, is denoted by $\maxk{c}{G}$. 
