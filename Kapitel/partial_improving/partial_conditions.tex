...
Let $c$ be a condition in ANF and $d$ be the subcondition of $c$ at layer $0 \leq k \leq \nl(c)$. 
...
The biggest $k$ with $G \models_k c$ such that no $j >k$ with $G \models_j c$ exists is denoted by $c_{\max}$.
\end{definition}
...
The set of biggest partially satisfying conditions of $c$ is denoted by $\mathcal{P}_c$. 

The set of graphs $C'$ such that $\parcond(c_{\max},c,C')$ is a biggest partially satisfying condition is denoted by $\mathcal{G}_{c}$. 
