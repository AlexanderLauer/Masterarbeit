	...
	\item $c = \forall(a:C_0 \inj C_1, d)$ and $p: C_0 \to G$:
		\begin{enumerate}
			\item $p \models c$:
				$$\rep(c,p) = 0$$
			\item$p \not \models c$: 
			Let $Q$ be the set of morphisms $q: C_1 \to G$ with $p = q  \circ a$ and $q \not \models d$.
				$$\rep(c,p) := | \bigcup_{q \in Q} q(E_{C_1 \setminus C_0}) \cup q(\iso(C_1) \cap V_{C_1})| + \sum_{q \in Q} \rep(d,q)$$
		\end{enumerate}				
