...
Let $G$ be a graph and $c$ a constraint in EANF, such that $G \models_k c$ and no $j > k$ with $G \models_k c$ exists. 
The \emph{number of violations $\nvc(j,G)$ at layer $j$ in $G$} is defined as:
\begin{enumerate}
\item If $j < k$:
	$$\nvc(j,G) := 0$$
\item If $j = k$, let $d = \forall(a_k: C_{k+1} \inj C_{k+2},e)$ be the subcondition of $c$ at layer $j+1$.
$$\nvc(j,G) := \sum_{C \in \mathcal{G}_{j,c}} \sum_{C' \in \mathcal{U}_{C,C_{j+2}}} |\{q \mid q:C_{j+2} \inj G \wedge q \not \models \parcond(1,e,C')\}| $$
\item If $j > k$:
	$$\nvc(j,G) := \infty$$
\end{enumerate}
...
Let $G$ be a graph, $r$ a plain rule and $c$ a constraint in ANF with $G \models \parcond(k,c,C')$ and $G \not \models_j c$ for all $j <k$. 
Let $d = \forall(a_k: C_{k-1} \inj C_{k}, e)$ be the condition at layer $k$ of $c$. 
A transformation $t: G \Longrightarrow_{r,m} H$ is called \emph{direct minimal consistency improving} if $H \models_k c$ and 
\begin{equation}\label{direct_improving_1}
\begin{split}
	& \forall p: C_{k} \inj G(( p \models \parcond(1,d,C') \wedge \track_t \circ p \text{ is total}) \implies \track_t \circ p \models \parcond(1,d,C')) \wedge \\ & \forall p': C_{k} \inj H (\neg \exists p : C \inj G(p' = \track_t \circ p) \implies p' \models \parcond(1,d,C')) 
\end{split}
\end{equation}
and 
\begin{equation}\label{direct_improving_2}
\begin{split}
\bigvee_{C'' \in \mathcal{U}(C', C_{k+1})}\exists p': C_k \inj G \Big(&p \not \models \parcond(1,d,C'') \implies \big((\track_t \circ p' \text{is total } \wedge \\ &\track_t \circ p  \models \parcond(1,d,C''))  \vee \\ &\track_t \circ p \text{ is not total}\big)\Big).
\end{split}
\end{equation}
...
Let $k$ be such that $G\models_k c$ and no $j >k$ with $G \models_j c$ exists and let $C_{k+1} \subseteq C \subseteq C_{k+2}$ such that $G \models \parcond(k,c,C)$ and no $C \subset C' \subseteq C_{k+2}$ with   $G \models \parcond(k,c,C')$ exists. 
Let $d$ be the subcondition of $c$ at layer $k+1$.

\begin{enumerate}
\item \label{proof_direct_minimal_4} We show that equation (\ref{direct_improving_1}) implies that $\nvc(H,k) \leq \nvc(G,k)$. 
Assume that $\nvc(H,c) > \nvc(G,c)$. Therefore a morphism $p: C_k \to G$ with $p \not \models \parcond(1,d,C')$ for a $C' \in \mathcal{U}_C$ exists, such that either \ref{proof_direct_minimal_1} or \ref{proof_direct_minimal_2} is satisfied.
\begin{enumerate}
	\item \label{proof_direct_minimal_1}
		There does not exist a morphism $q : C_k \to G$ such that $p = \track_t \circ q$. 
	\item \label{proof_direct_minimal_2}
		There does exist a morphism $q': C_k \to G$ with $q' \models \parcond(1,d,C')$ and $p = \track_t \circ q'$. 
\end{enumerate}
This is a contradiction, since if \ref{proof_direct_minimal_1} is satisfied, $q$ does not satisfy the first part of equation \ref{direct_improving_1} and if \ref{proof_direct_minimal_2} is satisfied $q'$ does not satisfy the second part of equation \ref{direct_improving_1}.

\item \label{proof_direct_minimal_3} Since (\ref{direct_improving_2}) is satisfied, a morphism $p:C_k \inj G$ with $p \not \models \parcond(1,d,C'')$, such that either $\track \circ p$ is total and $p  \models \parcond(1,d,C'')$ or $\track \circ p$ is not total.
In both cases the following holds $$\track \circ p \notin \{q \mid q:C_{k+2} \inj H \wedge q \not \models \parcond(1,e,C')\}.$$
