...
Let $G$ be a graph and $c$ a constraint in EANF, such that $G \models_k c$. 
...
$$\nvc(k,G) := \sum_{C \in \mathcal{G}_{k,c}} \sum_{C' \in \mathcal{U}_{C,C_{k+2}}} |\{q \mid q:C_{k+2} \inj G \wedge q \not \models \parcond(1,e,C')\}| $$
\item For $j > k$:
	$$\nvc(k,G) := \infty$$
\end{enumerate}
