...
That means, given a graph $G$ and a constraint $c$ and let $k = \maxc{G}$ and $\forall(a_{k+1}: C_{k} \inj C_{k+1}, \exists(a_{k+2}: C_{k+1} \inj C_{k+2},d) =: \cond(k+2,c)$.  
A transformation $ G \Longrightarrow H$ is considered as (direct) consistency increasing if $\maxc{G} \leq \maxc{H}$, i.e. the satisfaction up to layer is not decreased, and at least one occurrence of $C_{k+2}$ has been deleted or elements of 
