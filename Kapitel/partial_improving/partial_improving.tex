...
For the special case that $k = \nl(c) -1$ and $c$ ends with $\forall(a_k:C_{k-1} \inj C_k,\false)$ it can be shown in a similar way that  (\ref{direct_improving_1}) and (\ref{direct_improving_1_2}) imply that 
...
In this chapter, the notions of (direct) consistency increasing are compared to the already known concepts of consistency guaranteeing \cite{habel2009correctness} and (direct) consistency increasing and sustaining \cite{kosiol2022sustaining} in order to reveal relationships between them and secure that (direct) increasing is indeed a new notion of consistency. 
