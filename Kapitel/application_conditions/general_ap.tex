...
Let us start by introducing consistency-maintaining application conditions, i.e., a rule equipped with this application condition is direct consistency-maintaining at a given layer. 
We will also introduce an application condition such that any rule equipped with it is a consistency-maintaining rule.

The maintaining application condition has a odd parameter $k$ which specifies which graphs of a constraint are to be considered. 
In particular, only the graphs $C_j$ with $0 \leq j \leq k+3$ are considered.
Note that there is no graph with $\kmax$ odd and $\kmax \neq \nlvl(c)-1$. 
...
\begin{enumerate}
	...
	\item \emph{No existentially destroyed ($\remain{k}{}$):}
		 This application condition checks that no occurrences of existentially bound graphs $C_j$ with $2 \leq j \leq k+1$ are removed. It corresponds to the existentially condition, since a rule  that satisfies this application condition also satisfies the existentially condition when applied to a graph with $\kmax = k$.
	\item \emph{No universally inserted ($\ins{k}{}$):}
		This application condition checks that no occurrences of universally bound graphs $C_j$ with $1 \leq j \leq k+2$ are inserted. It corresponds to the universally and the inserting conditions, in the sense that, a rule that satisfies this application condition also satisfies the universally and inserting conditions when applied to a graph with $\kmax = k$.
	\item \emph{No violation inserted ($\wors{k}{} $):} 
		...
		It corresponds to the the deleting condition in the sense that a rule that satisfies this application condition also satisfies the deleting condition when applied to a graph with $\kmax = k$.
...
If $k = \nlvl(c) -1$, a rule equipped with this application condition always satisfies the deleting condition when applied to graphs with $\kmax = k$.  Therefore, if $k \geq \nlvl(c)-2$, we set the application condition to $\true$.
		\end{enumerate}

Recall that given a constraint $c$ in UANF, each subcondition $\scond{k}{c}$
is a condition over the graph $C_k$ and the morphism is denoted by $a_k$.
...
\begin{definition}[\textbf{consistency maintaining application condition}]
	...
	\begin{enumerate}
		...
		\item No universally inserted:
			Let $U$ be the set of all universally bound graphs $C_j$ with $1 \leq j \leq
...
\begin{example}
	\begin{enumerate}
		\item
			Consider the constraint $c$ given in Figure \ref{fig:appl_cond} and the rule \emph{\texttt{removeFeature}}. The conditions $\remain{-1}{\emph{\texttt{removeFeature}}}$, $\remain{1}{\emph{\texttt{removeFeature}}}$ and $\remain{3}{\emph{\texttt{removeFeature}}}$ are also given in Figure  \ref{fig:appl_cond}; $\remain{-1}{\emph{\texttt{removeFeature}}}$ is equal to true because there is no existentially bound graph $C_0$. $\remain{1}{\emph{\texttt{removeFeature}}}$ checks that no occurrences of $C_2$ are inserted, while $\remain{3}{\emph{\texttt{removeFeature}}}$ checks that no occurrences of $C_2$ and $C_4$ are inserted. Obviously, $\remain{1}{\emph{\texttt{removeFeature}}}$ is contained in $\remain{3}{\emph{\texttt{removeFeature}}}$. Note that there are are conditions in $\remain{3}{\emph{\texttt{removeFeature}}}$ that imply each other. For example, the first condition implies the second and third. 
So they can be removed.
		\item
		Again consider constraint $c$ given in Figure \ref{fig:appl_cond} and 
		the rule \emph{\texttt{assignFeature}}. The condition $\ins{-1}{\emph{\texttt{assignFeature}}}$ is equal to $\true$ since \emph{\texttt{assignFeature}} cannot create a node of type \emph{\texttt{Class}}. The condition $\ins{1}{\emph{\texttt{assignFeature}}}$ is given in Figure \ref{fig:nui}.
		The condition  $\ins{3}{\emph{\texttt{assignFeature}}}$ is equal to $\ins{1}{\emph{\texttt{assignFeature}}}$ since there is no universally bound graph $C_j$ with 
		$j > 3$.
		\item
...
\begin{theorem}\label{thm_maintaining_ac}
	...
	Every rule $\rho = (\ac \wedge \main{k}{\rho'}, \rho')$ with 
	$-1 \leq k < \nlvl(c)$ odd is a consistency maintaining rule at layer $k$ 
	w.r.t. $c$.   
\end{theorem} 
\begin{proof}
	...
	We show that $t$ is a direct consistency maintaining transformation w.r.t. $c$.
	 ...
	 We show that $t$ satisfies the deleting, inserting, universally and existentially conditions.
	
	\begin{enumerate}
		\item 
			 Assume that $t$ does not satisfy the deleting condition.
			 Then $\kmax < \nlvl(c) -1$,
			 ...
			 Therefore, there is an overlap $P$ of $L$ and $C'$ such that $i_{C'}^P \circ a^r_{k+2} 
		...
		\item 
			Assume that $t$ does not satisfy the inserting condition. Let
		...
		\item 
			Assume that $t$ does not satisfy the universally condition. Then, there is a 
		...
		\item 
			Assume that $t$ does not satisfy the existentially condition.  There is a 
	...
	\end{enumerate}
	It follows that $\rho$ is a consistency-maintaining rule at layer $k$
	w.r.t. $c$.
\end{proof}
...
\begin{theorem}
	...
	is a consistency-maintaining rule w.r.t. $c$.
\end{theorem}
...
\begin{proof}
	...
	$-1 < j \leq \nlvl(c)-1$.
	The set of intermediate graphs always contains both graphs on which this set was built, so $\wors{i}{\rho}$ contains the condition 
	$$ \bigwedge_{P' \in 
			\mathbf{P}_{C_{i+3}}} \neg \exists(i_L^{P'}: L \inj P', \true)$$
	which checks that no occurrence of $C_{i+3 }$ is deleted. Therefore 
...
It follows that $\rho$ is a consistency-maintaining rule at layer $k$ for all 
$-1 \leq k < \nlvl(c)$. Hence, $\rho$ is a consistency-maintaining rule w.r.t. $c$.
\end{proof}
The \emph{extended overlap} will be a useful tool for our consistency-increasing application conditions.
Intuitively, given an overlap and a morphism $a$, the overlap is extended such that one overlap morphism satisfies $\exists(a, \true)$.
...
Using extended overlaps, we will be able to check whether an violation has been 
removed.
Let us now consider consistency-increasing application conditions. 
For these we will use the maintaining application conditions described above.
...
Again, the increasing application condition has the odd parameter $-1 \leq k < \nlvl(c)-1$, which specifies which constraint graphs are to be considered. Note that $k$ must not be $\nlvl(c)-1$, since all graphs with $\kmax = \nlvl(c)$ satisfy the constraint. Therefore for these graphs there can be no consistency increasing transformations.
It also has a second parameter $C'$ which is an intermediate graph of $C_{k+2}$ and $C_{k+3}$ if $C_{k+3}$ exists and is set to $C_{k+2}$ otherwise. 
Again, a rule equipped with this application condition is a consistency-increasing rule at layer $k$.
...
\begin{enumerate}
	...
	\item \emph{Violation exists} ($\nex()$): 
		This condition checks that there is a violation at the match, i.e. there is an occurrence $p$ of $C_{k+2}$ with $m(L) \cap p(C_{k+2}) \neq \emptyset$ not satisfying $\exists(C', \true)$. There is a special case when $k = \nlvl(c)-2$ and $c$ ends with $\forall(C_{\nlvl(c)}, \false)$, it is sufficient to check only that $m(L) \cap p(C_{k+2}) \neq \emptyset$.  If there are no such occurrences, the transformation cannot be consistency-increasing.
...
\begin{enumerate}
	...
	\item If $\scond{k+2}{c} = \false$, i.e. $k = \nlvl(c)-2$,
		...
		\begin{cases} 
			...
			\false &\text{otherwise}.
		\end{cases}$$
	\item Otherwise, let $P'$ be the graph derived by the transformation $P \Longrightarrow_{\rho,m} P'$. Then, $P'$ is an overlap of $R$ and $C_{k+2}$.
If this transformation does not exist, we set $\rep(P,C') := \false$ and otherwise 
$$\rep(P,C') := \bigvee_{Q \in \eol(P',i_{C_{k+2}}^{P'}, a^r)} \shift(\forall (i_R^{Q}: R \inj P', 
				\exists(i_{P'}^Q:P' \inj Q,\true)), \rho).$$
\end{enumerate}
\end{enumerate}
...
an occurrence of $p: C_{k+2} \inj G$ that does not satisfy $\exists(C', \true)$ is either removed or satisfies $\exists(C', \true)$ in the derived graph.
For all smaller improvements, i.e. a similar improvement for a subgraph $C'' \in \ig{C_{k+1}}{C'}$ of $C'$, $\incr{k}{C'}{\rho}$ would be evaluated as $\false$.
For any larger improvements, i.e. the same improvement for a supergraph $C'' \in \ig{C'}{C_{k+2}}$ of $C'$, $\incr{k}{C'}{\rho}$ will also be evaluated as $\false$ if the repaired occurrence of $C_{k+2}$ satisfies $\exists(C', \true)$.
In both cases, the application condition would prohibit the transformation, even though it would be consistency-increasing.
To solve this problem, several application conditions could be combined by
$$\bigvee_{C' \in \ig{C_{k+1}}{C_{k+2}}} \incr{k}{C'}{\rho}.$$ 
This application condition will be evaluated as $\true$ if the cases described above occur, with the drawback that this results in a huge condition, even if duplicate conditions are removed.
...
For example, any of these application conditions with the rule \texttt{moveFeature} and the constraint $c_1$ will be equivalent to $\false$; the maintaining part of the condition will always be evaluated as $\false$, since \texttt{moveFeature} always removes occurrences of the existentially bound graph $C_2^1$.
...
For $\rem()$ and an occurrence $p$ of the universally bound graph $C_{j}$, by checking whether there exist two occurrences $p_1,p_2$ of $C_{j}$ with $p = p_1 \circ a_j = p_2 \circ a_j$, and for $\nin()$, by checking whether an introduced occurrence $p$ of $C_j$ satisfies $\exists(C_{j+1}, \true)$.
...
Also, the application conditions constructed above become more and more restrictive as $k$ increases, since the number of conditions also increases.
...
\emph{\texttt{assignFeature}}.
There is only one overlap $P$ of $L$ and $C_1^1$ which is shown in Figure Consider Figure \ref{fig_increasing_1}. 
The $\nex(P,C')$ and $\nex(P, C')$ parts of $\incr{-1}{C'}{\emph{\texttt{assignFeature}}}$ with $C' = C_2^2$ and $C' = C_2^1$, respectively, are also given in Figure \ref{fig_increasing_1}.
	...
	The application increasing application condition of this rule and at layer $-1$ with $C_1$ of the constraint is also given in this Figure.
\end{enumerate}
Note that we have already used the $\shift$ operator in all of these conditions.
\end{example}
...
\begin{theorem}
	...
	$C' = C_{k+2}$ otherwise is a consistency-increasing rule at layer $k$ 
	w.r.t. $c$.
...
\begin{proof}
	...
	be given. 
	Since  $\incr{k}{C}{\rho}$ contains $\main{k}{\rho}$, $t$ is a consistency maintaining transformation at layer $k$ according to Theorem \ref{thm_maintaining_ac}. It remains to show that $t$ satisfies 
	the general and special increasing conditions respectively.
	
	
	\begin{enumerate}
		\item	
			If $k = \nlvl(c) -2$, i.e. $c$ ends with a condition of the form $\forall(C_{\nlvl(c)},\false)$, assume that $t$ does not satisfy the special increasing condition. Then there is no morphism $p: C_{k+2} \inj G$ such that $\track_t \circ p$ is not total.
			So there is no overlap $P$ of $L$ and $C_{k+2}$ with $i_L^P(L\setminus K) \cap i_{C_{k+2}}^P(C_{k+2}\setminus C_{k+1} ) \neq \emptyset$.
			Since, $k = \nlvl(c) -2$ it follows that  $\rep(P,C') = \false$ and $m \not \models \incr{k}	{C}{\rho}$. This is a contradiction.
		
		\item 
			Otherwise let $P \in \overlay(L, C_{k+2})$. We show that 
			$m \models \vFound{}{P}{C} \wedge \vRepaired{}{P}{C}$ implies  that $t$ satisfies the general increasing condition.
			...
			If $i_L^P(L \setminus K) \cap i_{C_{k+2}}^P(C_{k+2} \setminus C_{k+1}) \neq \emptyset$,
			$\track_t \circ q$ is not total and the general increasing condition is satisfied. 
			
			Otherwise,  if $\track_t \circ q$ is total, and there is 
			a morphism $p: P' \inj H$ with 
			$\track_t\circ q = p \circ i_{C_{k+1}}^{P'}$.
			Since $m \models \vRepaired{}{P}{C}$,  all morphisms $p \circ 
			i_{C_{k+2}}^{P'}$ with $n = p \circ i_R^Q$ satisfy 
			$\exists(C', \true)$. Therefore, $\track_t \circ q \models 
			\exists(C', \true)$ and the general increasing condition is satisfied.
	\end{enumerate}	 
	In summary, $\rho$ is a consistency increasing rule at layer $k$.
\end{proof}