...
$$\rep(P,C') :=  \begin{cases}
				...
				\bigvee_{Q \in \eol(P', a^r)} \shift(\exists (i_R^{Q}: R \inj Q, \true), \rho) & \text{otherwise.}
				\end{cases}$$
...
\begin{theorem}
	...
	$\ac' = \ac \wedge \incr{k}{C}{\rho}$ and $0 \leq k < \nlvl(c)$ being even 
...
\begin{proof}
	...
	be given. 
	Since $\main{k}{\rho}$ is contained in $\incr{k}{C}{\rho}$, $t$ is a 
	consistency maintaining transformation at layer $k$ with Theorem 
...
Assume that (\ref{direct_improving_2}) does not hold. 
