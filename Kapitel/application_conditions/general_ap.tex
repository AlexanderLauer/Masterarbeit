...
We will now present consistency-maintaining and consistency-increasing application conditions at layer for general rules. 
That means, a rule equipped with these application conditions is consistency-maintaining or consistency-increasing at layer respectively. We will also introduce an application condition such that any rule equipped with it is indeed a consistency-maintaining rule.

Let us start with the consistency-maintaining application condition. 
The maintaining application condition has a odd parameter $-1 \leq k < \nlvl(c)$ which specifies which graphs of a constraint are to be considered. 
...
\begin{definition}[\textbf{consistency maintaining application condition}]\label{def_main}
...
We are aware that there are optimisations for $\ins{k}{} $ and $\remain{k}{\rho'}$ since not all overlaps need to be considered which  \cite{behr2020efficient}. But for the simplicity of our definition we did not implement these optimisations. 
\begin{example}
	\begin{enumerate}
		\item
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item 
			Assume that $t$ does not satisfy the \emph{no satisfied layer reduction by deletion} formula.  There is a 
...
\begin{proof}
	...
	$-1 < j \leq \nlvl(c)-1$.
	The set of intermediate graphs always contains second graphs on which this set was built, so $\wors{i}{\rho}$ contains the condition 
	$$ \bigwedge_{P' \in 
			\mathbf{P}_{C_{i+3}}} \neg \exists(i_L^{P'}: L \inj P', \true)$$
...
It also has a second parameter $C'$ which is an intermediate graph of $C_{k+2}$ and $C_{k+3}$ if $c$ contains a graph $C_{k+3}$, i.e. $k < \nlvl(c)-2$ and $C'$ is set to $C_{k+2}$ otherwise. 
...
For example, any of these application conditions with the rule \texttt{moveFeature} and the constraint $c_1$ will be equivalent to $\false$; the maintaining part of the condition will always be evaluated to $\false$, since \texttt{moveFeature} always removes occurrences of the existentially bound graph $C_2^1$.
