...
We first start with an algorithm which calculated $\kmax$ given a graph $G$ and a constraint $c$ in UANF, it is shown in Algorithm \ref{Algo_kmax}.  
...
$C_{\kmax+2}$ has a conflict with $C_i$ and this could lead to a decrease of the largest satisfied layer. Again, the algorithm finds all inserted occurrences of universally bound graphs (line 11). Again, in both cases, the algorithm only finds potentially increasing occurrences at the respective layer w.r.t. $c$. 
...
Since it is unclear for which  of these occurrences a deletion or extensions leads to increase of the largest satisfied layer, we select one at random. 
...
\begin{theorem}
Given a graph $G$,  a circular conflict free condition $c$ in UANF and 
...
\begin{proof}
	...
	Since $c$ is free of circular conflict, $M'$ cannot contain any occurrences of $C_{\kmax +2}$, otherwise $C_j$ would have cause a (transitive) conflict for $C_{\kmax +2}$ and therefore $C_{\kmax +2}$ has a circular conflict. Therefore no occurrences of $C_{\kmax +3}$ are destroyed and no occurrences of $C_{\kmax+2}$ are inserted.
	In addition, $C_{\kmax +2}$ causes a (transitive) conflict for $C_i$, and repairing any $p \in M'$ will not lead to insertion of an occurrence of $C_{\kmax +2}$ or removal of an occurrence of $C_{\kmax +3}$.
	
	Since $c$ is circular conflict free, there must exist graphs $C_i$, such that $C_i$ causes no conflict with any other graph $C_{i'}$ and $C_{\kmax+2}$ causes a (transitive) conflict for $C_i$. 
