...
\begin{definition}[\textbf{conflict graphs, circular conflicts}]
	...
	A constraint $c$ causes a \emph{transitive conflict w.r.t. $\mathcal{R}$} for $c'$ if the 
...
\begin{theorem}
	Given rule set $\mathcal{R}$, a graph $G$ and a satisfiable set of constraints in UANF $\mathcal{C}$, which is circular conflict free w.r.t. $\mathcal{R}$.
