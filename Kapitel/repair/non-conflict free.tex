...
Then $c$ has a conflict with $c'$ w.r.t. $\mathcal{R}$ if a repairing sequence 
$$C_k = G_0 \Longrightarrow_{\rho_1,m_1} \dots \Longrightarrow_{\rho_n,m_n}
	G_n$$
	exists for a graph $C_k$ of $c$ such that the concurrent rule of that 
...
It states that applying a repairing sequence to a constraint $c$ cannot destroy the satisfaction of $c'$ if $c$ has no conflict with $c'$.

\begin{lemma}\label{lemma_preserving}
	Let two constraints $c$ and $c'$ in UANF be given such that $c$ has no conflict with $c'$ w.r.t. to a set of rules $\mathcal{R}$.
Then the concurrent rule $\rho$ of any repairing sequence for $c$ is a $c'$-preserving rule.
\end{lemma}
\begin{proof}
...
The \emph{conflict graph} for a set of constraints and \emph{circular conflicts} for it are defined in a similar way to the conflict graph and circular conflicts for a constraint. 
...
\begin{definition}[\textbf{conflict graphs, circular conflicts}]
	Given a set of constraints $\mathcal{C}$ in UANF. The \emph{conflict graph} of $\mathcal{C}$ is constructed in the following way. For each constraint $c \in \mathcal{C}$ there is a node. If there is a conflict between $c$ and $c'$, there exists an edge $e$ with $\src(e) = c$ and $\tar(e) = c'$.
...
A set of constraints $\mathcal{C}$ is called \emph{circular conflict free} if every constraint in $\mathcal{C}$ is circular conflict free and $\mathcal{C}$ contains no circular conflicts.
\end{definition}
...
\end{definition}
It is well known that every directed acyclic graph has a topological ordering, and therefore every conflict graph of a circular conflict-free set of constraints also has a topological ordering.
...
\begin{theorem}
	Given a graph $G$, a satisfiable, circular  conflict-free set of constraints in UANF, $\mathcal{C}$, and a set of rules $\mathcal{R}$.
...
\begin{proof}
	...
	Let $(c_1, \ldots, c_n)$ be a topological ordering of $\mathcal{C}$. Then no constraint $c_j$ with $j \neq 1$ has a conflict with $c_1$, and by Lemma \ref{lemma_preserving} it follows that the concurrent rule of every repair sequence for every $c_i$ with $2 \leq i \leq n$ is a $c_1$-preserving rule.
 ...
 \ref{Algo_non-conflict_free}. Since each concurrent rule of each repairing sequence of $c_{m+1}$ is a $c_i$-preserving rule for all $1 \leq i \leq m$, the application of repairing sequence can be replaced by its concurrent rule, and Algorithm \ref{Algo_non-conflict_free} applies only these concurrent rules, it follows that $H \models c_i$ for all $1 \leq i \leq m+1$.  
