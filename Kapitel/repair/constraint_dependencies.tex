...
\begin{definition}[\textbf{conflicts within conditions}]\label{def_conflicts}
	...
	A existentially bound graph $C_k$ has a conflict with an universally bound 
	...
	A universally bound graph $C_k$ has a conflict with an existentially bound 
	graph $C_j$ if there is a transformation $t: G \Longrightarrow_{\rho} H$ with 
...
\begin{definition}[\textbf{conflict graph, circular conflicts}]\label{conflicts_within}
	...
	For every $0 \leq k < \nlvl(c)$ there is a node labelled $k$.	If there is a conflict between $C_k$ and $C_j$, there is an edge $e$ with $\src(e) = k'$ and $\tar(e) = j'$ if either $k = k'$ or $k = k'+1$, either $j = j'$ or $j = j'+1$ and $j' \neq k'$.
	
	A graph $C_k$ has a \emph{transitive conflict} with $C_j$ if there exists a path from $k$ to $j$.
...
\begin{example}
	...
	rule $\rho = \rle{C_1}{\id}{C_1}{a_1}{C_2}$ has been applied and there is a newly inserted occurrence of $C_1$ that does not satisfy
	$\exists(C_2, \true)$. 
...
We will now present two characterisations of conflicts. One based on overlaps and the other based on rules. 
For $C_k$, which is existentially bound and $C_j$, which is universally bound, the overlap-based characterisation checks whether for each overlap of $C_k$ and $C_j$, such that the overlap morphisms restricted to $C_k \setminus C_{k+1}$ and $C_j$ overlap, the rule that only deletes $C_k\setminus C_{k-1}$ is applicable.
...
\begin{enumerate}
	\item 
		...
		$c$. Then, $C_k$ has a conflict with $C_j$, if and only there is an overlap $P 
	...
	\item 
		...
		$c$. Then, $C_k$ has a conflict with $C_j$ if there is an overlap $P \in \ig{C_k}{C_j}$ with 
		...
		\setminus C_{j-1})\neq \emptyset $ is applicable at match $i_{C_k}^P$.
...
The graph $p(C_j) \cup n(C_k)$ is the overlap we are looking for, and the rule $\rho^{-1} = \rle{C_{k}}{a_{k-1}}{C_{k-1}}{\id}{C_{k-1}}$ must be applicable at the match $n$. 
\\
...
Then the inverse transformation of $t: P \Longrightarrow_{\rho,i_{C_k}^P} H$ is the transformation we are looking for and $C_k$ has a conflict with $C_j$. 

\item
\enquote{$\Longrightarrow$}: Let $C_k$ be an universally bound graph that has a conflict with an existentially bound graph $C_j$. 
Then, there is a transformation $t: G \Longrightarrow_{\rho} H$ with $\rho = \rle{C_k}{a_{k-1}^r}{C}{\id}{C}$ and $C \in \ig{C_{k-1}}{C_k}$ such that $\track_t \circ p$ is no total for an occurrence $p : C_j \inj G$. 
The graph $p(C_j) \cup m(C_k)$ is the overlap we are looking for and  $i_{C_k}^P(C_k \setminus C) \cap i_{C_j}^P(C_j\setminus C_{j-1}) \neq \emptyset$ must hold since $\rho$ only deletes elements of $C_k\setminus C_{k-1}$.
\\
\enquote{$\Longleftarrow$}:
Let $C_k$ be universally and $C_j$ existentially bound such that there is an  overlap $P \in \overlay(C_k,C_j)$ with $i_{C_k}^P(C_k \setminus C_{k-1}) \cap i_{C_j}^P(C_j\setminus C_{j-1}) \neq \emptyset$  so that a rule $\rho = 	\rle{C_k}{a_{k-1}^r}{C}{\id}{C}$ with $C \in \ig{C_{k-1}}{C_k}$ is applicable at match $i_{C_k}^P$.
Then, the transformation of $t: P \Longrightarrow_{\rho, i_{C_k}^P} H$ is the transformation we are looking for and $C_k$ has a conflict with $C_j$.
\end{enumerate}
\end{proof}
