...
Again, this can lead to the insertion of new violations or to a reduction of the largest satisfied layer.
...
If this is not possible, there is no a transformation as described in Definition \ref{conflicts_within}.
...
\begin{lemma}\label{basic_conflict}
	...
	\begin{enumerate}
		...
		\item
			...
			of $c$. Then, $C_k$ has a conflict with $C_j$ if and only if a
			rule
...
\begin{proof}
	\begin{enumerate}
		...
		\true)$ .
			...
			\rho} H$ that does not satisfy the no satisfied layer reduction by insertion formula and an occurrence of $C_j$ has been removed by $t$.
			It follows that  $C_k$ causes a conflict for $C_j$.
