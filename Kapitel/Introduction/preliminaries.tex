...
\begin{definition}[\textbf{subgraph}]
	...
	exists. $G$ is called a \emph{proper subgraph of $H$} if the $p$ is not bijective. 
\end{definition}
Note that since the inclusion can also be surjective, by this definition every graph $G$ is a subgraph of itself.

\begin{definition}[\textbf{intermediategraph}]
	Let $G$ and $H$ be graphs such that $G$ is a subgraph of $H$. 
	A graph $C$ is called an \emph{intermediategraph} of $G$ 
...
Note that $(H, i_{G_1}, i_{G_2})$ with $i_{G_1}$ and $i_{G_2}$ being jointly surjective and  $i_{G_1}(G_1) \cap i_{G_2}(G_2)= \emptyset$ could also be considered as an overlap of $G_1$ and $G_2$.
...
It holds that $i' \circ p^r = p \circ i =  i' \circ q^r$ and $p^r = q^r$ follows because $i'$ is injective.  
...
\begin{enumerate}
	...
	\item $\exists(a_0:C_0 \inj C_1,d)$ is a graph condition over $C_0$ if $a_0$ is a inclusion and $d$ is a graph condition over $C_1$. 
...
\begin{definition}[\textbf{shift over morphism}]
...
\begin{definition}[\textbf{shift over rule}]\label{def:shift}
	...
	application condition $\ap$ of $\rho$ be given. The \emph{shift of $\ap$ over 
	...
	\begin{enumerate}
		...
		\item 
			...
			$\rho' = \rle{C_0}{g}{D}{h}{C_0'} $ is the rule derived in Figure 
			\ref{fig_left} by applying $\rho^{-1}$ at match $a_0$.
...
\begin{definition}[\textbf{track morphism}]
	...
	The \emph{track morphism}, $\track_t: G \dashrightarrow H$, of $t$ is 
...
\begin{lemma}[\cite{kosiol2022sustaining}]
	Let a transformation $t: G \Longrightarrow G$ with transformation morphisms $g: D \inj G$, $h: D \inj H$ and an occurrence $p: C \inj G$ of a graph $C$ be given. Then,
	\begin{enumerate}
...
\begin{definition}[\textbf{new and removed occurrences}]
	Given a transformation $t:G \Longrightarrow H$ and a graph $C$.
	An occurrence $p: C \inj G$ is called a \emph{removed occurrence of $C$ by $t$} if $\track_t \circ p$ is not total. 
	An occurrence $p' : C \inj H$ is called a \emph{inserted occurrence of $C$ by $t$} if there is no morphism $p: C \inj G$ such that $p' = \track_t \circ p$. 
\end{definition}
...
\begin{definition}[\textbf{concurrent rule}]
...
\begin{definition}[\textbf{consistency preserving and guaranteeing transformations}]
...
\begin{definition}[\textbf{consistency sustaining and improving transformations}]
	...
	If $c = \forall(a_0: \emptyset \inj C_1,d)$ is universally bound, $t$ is 
	...
	\models d\}|.$$
	The number of elements of these sets are called the \emph{number of 
...
\begin{definition}[\textbf{direct sustaining and improving transformations}]
	Let a constraint $c$ in ANF and a transformation $t: G \Longrightarrow_{\rho} 
	H$ be given. If $c$ is existentially bound, $t$ is called \emph{direct 
