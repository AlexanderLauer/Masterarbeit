...
existentially bound. 
Conditions of the form $\forall(a:C_0 \inj C_1, d)$ are called 
...
\begin{definition}[\textbf{semantic of graph conditions}]
	...
	\begin{enumerate}
		\item	
			If $c = \true$, $p \models c$.
		\item 
			If $c = \exists(a:C_0 \inj C_1,d)$, then $p \models c$
...
with $\exists(a: C_0 \inj C_1, \false)$ or $\forall(a:C_0 \inj
C_1, \true)$. We exclude these cases such that conditions in ANF
are only allowed to end with conditions of the type $\exists(a: 
C_0 \inj C_1, \true)$ or $\forall(a:C_0 \inj C_1, \false)$ since
it can be easily seen that every morphism $p: C_0 \inj G$ satisfies
$\forall(a:C_0 \inj C_1, \true)$ and does not satisfy $\exists(a: C_0 \inj C_1, \false)$. Therefore, these conditions can be replaced by $\true$ and $\false$ respectively.
