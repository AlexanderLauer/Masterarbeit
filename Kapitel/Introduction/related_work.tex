...
They also introduce a graph repair approach given a set of rules $\mathcal{R}$. 
Their approach can be used to repair a graph with rules from $\mathcal{R}$ if there is a repair program such that for every rule in the repair program there is an equivalent rule in $\mathcal{R}$.
\\ \\
