...
Firstly, either $k < \nlvl(c)-1$ or the constraints ends with a condition of the form $\exists(a: C \inj C', \true)$ and secondly, $k = \nlvl(c)-1$ and the constraint ends with a condition of the form $\forall(a: C_0 \inj C_1, \false)$.
...
with
\begin{enumerate}
\item Let $a^p: C_k \inj C'$ be a partial morphism of $a_{k+1}$ and $i_L$ and $i_P$ the inclusions of $L$ in $P$ and $P$ in $Q$, respectively.
	$$\nex(P,C') := 
			\begin{cases}
...
\item If $i_L^P(L \setminus K) \cap i_{C_k}^P(C_k) \neq \emptyset$, we set
$$\rep(P,C') := \true.$$
Otherwise, let $P'$ be the graph derived the transformation $P \Longrightarrow_{\rho,m} P'$. Then, $P'$ is an overlap of $R$ and $C_k$.
If this transformation does not exist, we set 	$\rep(P,C') := \false$.
Let $a^p:C_k \inj C'$ be a partial morphism of $a_{k+1}$, then 
...
Then, a morphism $p: C_k \inj G$ exists, such that $p \models \parcond(1,e,C')$, $\track_t \circ p$ is total and $\track_t \circ p \not \models \parcond(1,e,C')$ for a graph $C' \in \mathbf{G}$. 
Therefore, an overlap $P$ of $L$ and $C'$ such that $i_{C_k}^P \models \exists(a^p_k: C_k \inj C', \true)$ with $i_L^P(L \setminus K) \cap i_{C'}^P(C' \setminus C_k) \neq \emptyset$ exists and $m \models \exists(i_L: L \inj P, \true)$ holds. 
Thus $\nwo()$ and consequently also $\ap(k,C)$ cannot be satisfied. 

\item Assume that  (\ref{direct_improving_1_2}) does not hold and let

$$ d := \begin{cases} 
		\parcond(1,e,C_{k+1}) & \text{if $e \neq \false$} \\
		\false &\text{otherwise.}
	
	\end{cases}	
$$

  Then, a morphism $p': C_k \inj H$ with $p' \not \models d$ exists, such that no morphism $p :C_k \inj G$ with $\track_t \circ p = p'$ exists. 
Therefore, an overlap $P$ of $R$ and $C_k$ with $i_R(R\setminus K) \cap i_R(C_k) \neq \emptyset$ exists, such that $m \models \shift(\exists(i_R^P: R \inj P, \true), \rho)$ and $m$ does not satisfy $\nin()$. 

\item Assume that (\ref{direct_improving_2}) does not hold. 
Then, no morphism $p: C_k \inj G$ with $p \not \models \parcond(1,e,C')$, such that $\track_t \circ p$ is not total or $\track_t \circ p \models \parcond(1,e,C')$ and $\track_t \circ p$ is total exists, for any $C' \in \mathbf{G}$.
Then, no overlap $P$ of $L$ and $C_k$ with $i_L^P(L\setminus K) \cap i_{C_k}^P(C_k) \neq \emptyset$ exists, such that $m \models \nex(P,C)$.

Let $P$ be an overlap of $C_k$ and $L$ with $i_L^P(L \setminus K) \cap i_{C_k}^P(C_k) = \emptyset$.
If $e  = \false$, then $\rep(P,C) = \false$. 
Otherwise, if $m \models \nex(P,C) \wedge \rep(P,C)$, it holds that $i_{C_k}^P \not \models \parcond(1,e,C)$ and a  graph $Q \in \eol(P', a^p)$ exists, such that $m \models \shift(\exists(i_R^Q:R \inj Q, \true),\rho)$. 
In this case, $\track_t \circ i_{C_k}^P \models \parcond(1,e,C)$ follows and (\ref{direct_improving_2}) is satisfied. 

In total, $m \not \models \nex(P,C) \wedge \rep(P,C)$ follows for all $P \in \overlay(L,C_K)$ and therefore $m \not \models \ap(k,C)$.

\item Assume that (\ref{direct_improving_3}) does not hold.
Then, a morphism $p: C_j \inj H$ with $j < k$ and $j$ being odd exists, such that no morphism $p': C_j \inj G$ with $\track_t \circ p' = p$ exists. 
...
Then, a morphism $p :C_j \inj G$ with $j < k$ and $j$ being even exists, such that $\track_t \circ p$ is not total. 
...
\end{enumerate}
In total, if $\models \ap(k,C)$, then $t$ is a direct consistency increasing transformation.
\end{proof}
