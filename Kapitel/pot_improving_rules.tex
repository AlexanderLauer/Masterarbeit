...
Then, $\rho$ is called  \emph{basic increasing w.r.t $c$ at layer $k$} if $\rho$ is a non-consistency decreasing w.r.t $c$ up to layer $k$ and
...
\item If $\scond{k}{c}$ is existentially bound, there does exist a morphism $i: C_{k} \inj L$ such that 
	...
	for any $C'\in \ig{C_{k}}{ C_{k+1}}$, and for each transformation $t: G \Longrightarrow_{\rho} H$ it holds that
