...
The rule $\rho$ is called \emph{basic improving} w.r.t $c$ at layer $k$ with $C_k \subset P \subseteq C_{k+1}$, $p : C_k \inj P$ being the inclusion, and $k \in \{1,3, \ldots , \nl(c)\}$, if
...
\begin{enumerate}
\item \label{pot_min_impr_1} The rule $\rho$ deletes  elements of $C_k \setminus C_{k-1}$:
	\begin{equation}
				 L \subseteq C_k \wedge L \setminus K \neq \emptyset 
	\end{equation}
	Then, $\rho$ is called a \emph{deleting basic improving rule}.

\item \label{pot_min_impr_2} The rule $\rho$ creates an instance of $P$:
	\begin{equation}\label{pot_ins}
		L = C_k \wedge P \subseteq R
	\end{equation}
	and $p$ is a partial morphism of $r$. 
	Then, $\rho$ is  called an \emph{inserting basic improving rule}.
\end{enumerate}
...
$$\api(j, P) := \begin{cases}
				\exists\bigl(b: L \inj C_{k}, \neg \exists(a_{k+1}:C_{k} \inj C_{k+1}, \true)\bigr) &\text{, if j = k} \\
				\false &\text{, if } j \neq k.
				\end{cases}$$
with
$$ b(e) := \begin{cases}
...
$$\api(j, P) := \begin{cases}
				...
				\false	&\text{, if } j \neq k
				\end{cases}$$
with $b$ being a partial morphism of $a_{k+1}$.
...
Let a graph $G$, a constraint $c$ in EANF, with $G \models \parcond(\cmax, c, C)$ and $\parcond(\cmax, c, C) \in \mathcal{P}_c^G$, and a potentially minimal improving rule $\rho = L \xhookleftarrow{l} K \xhookrightarrow{r} R$ at layer $\cmax$ with $C_{\cmax+1}\subseteq P \subseteq C_{\cmax+2}$ be given. 
Then, $\rho' = (\rho, \api(\cmax+1,P))$ is a direct minimal consistency improving rule. 
\end{theorem}
\begin{proof}
Let $t:G \Longrightarrow_{\rho',m} H$ be a transformation, $k = \cmax+1$ and $e$ be the subcondition of $c$ at layer $k+1$. 
...
Since $\rho$ is a deleting rule, at least one element of $p(C_k)$ has been deleted by $t$ and therefore $\track_t \circ p$ is not total. 
