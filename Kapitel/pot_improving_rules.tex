...
\begin{enumerate}
\item If $C_k$ is universally bound, let $I$ be the set of all isolated nodes of $C_k$, $P$ be the set of all partial morphisms $p: C_k \inj R$, $m = r$, 
$$E = \bigcup_{p \in P} p(E_{C_k}) \cap (E_R \setminus E_K)$$ and 
$$V = \bigcup_{p \in P} p(V_{C_k}) \cap (V_R \setminus V_K).$$

\item If $C_k$ is existentially bound, let $I$ be the set of all isolated nodes of $C_k$, $P$ be the set of al morphisms $p:C_k \inj L$, $m = l$, 
$$E = \bigcup_{p \in P} p(E_{C_k}) \cap (E_L \setminus E_K)$$ and 
$$V = \bigcup_{p \in P} p(V_{C_k}) \cap (V_L \setminus V_K).$$
\end{enumerate}
Let $I$ be the set of all isolated nodes of $C_k$. Then, $\rho$ is called a \emph{non-consistency decreasing rule at layer $k$ w.r.t $c$} if $I \cap V = \emptyset$ and the following holds:
\begin{alignat*}{2}
	&\big(\neg \exists e \in E: \exists v,v' \in V_K: \src(e) = m(v) \wedge \tar(e) = m(v')\big) & \wedge \\
...
\begin{lemma}
Let a constraint $c$ in EANF  and a non-consistency decreasing rule $\rho$ at layer $k$ w.r.t $c$ be given. 
Then, it holds that: If $C_k$ is universally bound, for each transformation $t : G \Longrightarrow_{\rho} H$:
$$\forall p:C_k \inj H: \exists p':C_k \inj G: \track_t \circ p' = p$$
If $C_k$ is existentially bound, for each transformation $t: G\Longrightarrow_{\rho} H$:
 $$ \forall p:C_k \inj G: \track_t \circ p \text{ is total}$$
\end{lemma}
\begin{proof}

\end{proof}

\begin{definition}[\textbf{basic improving rule}]
Let a constraint $c$ and a plain rule $\rho = L \xhookleftarrow{l} K \xhookrightarrow{r} R$ be given. 
Then, $\rho$ is called a \emph{basic improving w.r.t $c$ at layer $k$} if $\rho$ is a non-consistency decreasing rule at layer $j$ w.r.t $c$ for all $0 \leq j \leq k$ and
...
\item If $C_k$ is universally bound: 
	...
	Then, $\rho$ is called a \emph{deleting basic improving rule}.
\item If $C_k$ is existentially bound:
	There does exist a morphism $i: C_k \inj L$ such that $i \not \models \exists (a_k^p: C_k \inj C', \true)$ and $\	r \circ l^{-1} \circ i \models \exists(a_k^p:C_k \inj C', \true)$ for any $C'\in \mathcal{U}(C_k, C_{k+1})$ and for each $C''\in \mathcal{U}(C_k,C_{k+1}$ with $i \models \exists(a_k^p:C_k \inj C'', \true)$ it holds that $r \circ l^{-1} \circ i \models \exists(a_k^p:C_k \inj C'',\true)$ . 
	Then, $\rho$ is called an \emph{inserting basic improving rule with $C'$.}
\end{enumerate}
...
\begin{definition}[\textbf{application conditions for basic improving rules}]
Let a constraint $c$ in EANF and a basic improving rule  $\rho = L \xhookleftarrow{l} K \xhookrightarrow{r} R$ w.r.t $c$ at layer $k$  with $C_{k} \subseteq P \subseteq C_{k+1}$ be given. 
We define the application condition for $r$ as:
 
\begin{enumerate}
\item If $\rho$ is a deleting potentially minimal improving rule:
$$\api(j, P) := \begin{cases}
				\bigvee_{P \in \overlay(L,C_k)} \bigwedge_{P' \in \eol(P, a_k)} \exists(i_L: L \inj P, \neg \exists(i_P: P \inj P', \true)) &\text{, if j = k} \\
				\false &\text{, if } j \neq k.
				\end{cases}$$
\item If $\rho$ is an inserting potentially minimal improving rule:				
$$\api(j, P) := \begin{cases}
				\neg \exists(a^p_k: L \inj P, \true) &\text{, if j = k}\\
				\false	&\text{, if } j \neq k
				\end{cases}$$
