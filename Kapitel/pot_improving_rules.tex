...
Let a constraint $c$ in UANF and a plain rule $\rho = L \xhookleftarrow{l} K \xhookrightarrow{r} R$ be given. 
Then, $\rho$ is called  \emph{basic increasing w.r.t $c$ at layer $k$} if $\rho$ is a non-consistency decreasing w.r.t $c$ up to layer $k$ and for each transformation $t: G \Longrightarrow_{\rho} H$ it holds that
	\begin{equation}\label{def_basicIncreasing}
	\forall p: C_k \inj G\big(p \models \exists(a_k^r: C_k \inj C'', \true) \implies \track_t \circ p \models \exists(a_k^r: C_k \inj C'', \true) \big)
	\end{equation}
	
	for all $C''\in \ig{C_k}{C_{k+1}}$.

\begin{enumerate}
\item If $\scond{k}{c}$ is universally bound, there does exist a morphism $i: C_{k+1} \inj L$ such that
\begin{equation*}
	\begin{split}
...
\item If $\scond{k}{c}$ is existentially bound, there does exist a morphism $i: C_{k} \inj L$ such that 
