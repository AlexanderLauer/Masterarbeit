...
\begin{document}
	...
	%	SClass-phantom/.style={text width={width(": Feature")},align=center,draw},
		...
		\node(name)[anchor = west] {conflict graph of $c_3$};
		\node(1) [circle, draw, below right= 30pt and 5pt of name.west,anchor = west] {$1$};
		\node(2) [circle, draw, right= 40pt of 1] {$2$};
		\path[->] (1) edge [bend left =20] (2);
		\path[->] (2) edge [bend left =20] (1);
		\node(C_32-out) [outside, fit={(name) (1) (2)}]{};
		
		\node(name2)[anchor = west, right = 20pt of name] {conflict graph of $c_4$};
		\node(12) [circle, draw, below right= 30pt and 5pt of name2.west,anchor = west] {$1$};
		\node(22) [circle, draw, right= 40pt of 12] {$2$};
